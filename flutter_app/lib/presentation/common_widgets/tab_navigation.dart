...
void goToTab(BuildContext context, int index) {
  ...
  switch (index) {
    case 0:
      target = isHost && currentUserId != null
          ? HostHomeView(currentUserId: currentUserId)
          : const HomeView();
      break;

    case 1:
      target = const TripsView();
      break;

    case 2:
      // Messages ya no requiere ChatApi; obtiene datos vía repos/Api.I()
      target = MessagesView(currentUserId: currentUserId ?? '');
      break;

    case 3:
    default:
      target = const AccountView();
      break;
  }
