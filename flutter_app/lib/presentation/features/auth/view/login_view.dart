...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleLogin() async {
    ...
    // Evita teclado abierto y dobles taps
    ...
    // 1) Actualiza el cliente central y la sesión INMEDIATAMENTE
    Api.I().setToken(token);
    context.read<AuthProvider>().signIn(userId: userId, token: token);

    // 2) Persiste el token en background (no bloquea navegación del Splash)
    unawaited(context.read<AuthViewModel>().saveToken(token));

    _snack('¡Bienvenido!');
    // Navegación la maneja el Splash tras el rebuild del MaterialApp
  }
