...
// import 'package:flutter_app/presentation/features/settings/view/currency_view.dart';
// import 'package:flutter_app/presentation/features/settings/view/legal_view.dart';
...
import 'package:flutter_app/presentation/features/app_shell/viewmodel/host_mode_provider.dart';
// ⬇️ Usa el import correcto según tu estructura real:
// import 'package:flutter_app/presentation/features/settings/view/profile_settings_view.dart'; // <- usa este si tu archivo está ahí
...
class AccountView extends StatelessWidget {
  ...
  // -------- Helpers de rol (usando UserService + Api.I()) --------
  Future<String?> _readRole(UserService users, String userId) async {
    final http.Response res = await users.getUserRaw(userId);
    if (res.statusCode != 200) return null;

    final j = jsonDecode(res.body);
    if (j is! Map<String, dynamic>) return null;

    // role: 'host' | 'renter' | 'both'  (o roles: ['host','renter'])
    final role = j['role'];
    ...
    if (roles is List) {
      ...
      if (set.containsAll({'host', 'renter'})) return 'both';
  ...
  Future<void> _handleHostToggle(BuildContext context, bool value) async {
    ...
    final usersSvc = UserService();

    // Leer rol actual
    String? role = await _readRole(usersSvc, userId);
    ...
    // Navegar a registro para promover a host
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const RegisterScreen()),
    );

    // Releer rol tras registro
    role = await _readRole(usersSvc, userId);
    if (role == 'host' || role == 'both') {
      hostProvider.setHostMode(true);
    } else {
      hostProvider.setHostMode(false);
