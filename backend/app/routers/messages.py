...
async def list_my_messages(
    skip: int = 0,
    limit: int = 100,
    other_user_id: Optional[str] = Query(None, description="Filtra la conversación con otro usuario específico"),
    only_unread: bool = Query(False, description="Solo mensajes no leídos para el usuario actual"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user_from_token),
):
    """
    Listar mensajes del usuario actual (inbox + enviados).
    - Si `other_user_id` está presente, devuelve solo el hilo entre ambos usuarios.
    - Si `only_unread` está en True, filtra mensajes recibidos y no leídos.
    """
    svc = MessageService(db)

    if other_user_id:
        await _ensure_user_exists(db, other_user_id)
        return await svc.list_thread(
        ...
        return await svc.list_messages_for_user(
...
async def get_thread_with_user(
    ...
    """Obtener el hilo de mensajes entre el usuario actual y `other_user_id`."""
    await _ensure_user_exists(db, other_user_id)
    svc = MessageService(db)
...
async def send_message(
    ...
    if payload.receiver_id == current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No puedes enviarte un mensaje a ti mismo"
        )
...
async def mark_message_as_read(
    ...
    if msg.receiver_id != current_user.user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Solo el receptor puede marcar como leído")
...
async def update_message(
    ...
    *Opcional*: podrías validar ventana de edición por tiempo en el servicio.
    """
    svc = MessageService(db)
    msg = await svc.get_by_id(message_id)
    if not msg:
    ...
    if msg.sender_id != current_user.user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Solo el emisor puede editar el mensaje")

    updated = await svc.update_message(message_id=message_id, content=payload.content)
    return updated
...
async def delete_message(
    ...
    Si deseas “borrado lógico” para el receptor, manéjalo en el servicio.
    """
    svc = MessageService(db)
    msg = await svc.get_by_id(message_id)
    if not msg:
    ...
    if msg.sender_id != current_user.user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Solo el emisor puede eliminar el mensaje")
...
async def mark_thread_as_read(
    ...
    """
    Marcar como leídos todos los mensajes que el usuario actual ha recibido
    en el hilo con `other_user_id`.
    """
    await _ensure_user_exists(db, other_user_id)
    svc = MessageService(db)
